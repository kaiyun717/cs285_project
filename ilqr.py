...
def ilqr_forwards(dynamics, x0, xbar, ubar, K, k, horizon, scale):
    ...
    for t in range(horizon):
        ...
        x, cost, residuals, Fx, Fu, Rx, Ru, rx, ru = dynamics(x, u, t=t, is_last=(t==horizon-1))
...
def ilqr_backwards(rbar, derivs, horizon, regularization, Quu_min, ubar, verbose=False):
    ...
    for t in reversed(range(horizon)):
        Luu = np.eye(ubar[-1].shape[0]) * 1e-3 # Rus[t].T @ Rus[t]
        Lxx = Rxs[t].T @ Rxs[t]
        Lxu = Rxs[t].T @ Rus[t]
        Lx = rbar[t] @ Rxs[t] + rxs[t]
        Lu = Luu @ ubar[t] # rbar[t] @ Rus[t] + rus[t]
...
def do_ilqr(dynamics, x0, d_state, d_control, d_residual, horizon, num_iters=10, num_line_search_iters=10, regularization_init=1e-3, Quu_min=1e-3, verbose=False, ubar=None, xbar=None, K=None):
    if K is None:
