...
def sample_planar(sample_size, 
    ...
    obs_size = env.observation_space.shape[0]   # 6*2 for obstacles, 1*2 for state
    act_size = env.action_space.shape[0]
    ...
    for i in trange(sample_size):
        ...
        if apply_control:
            # u = np.random.uniform(low=-env.max_step_len,
            #                       high=env.max_step_len,
            #                       size=2)
            u = env.action_space.sample()
        else:
            u = np.zeros((act_size,))
        ...
        while True:
            # print(state[-2:].squeeze(axis=1).shape)
            # print(u.shape)
            if env.is_valid_action(state[-2:].squeeze(axis=1), u):
                break
            else:
                # u = np.random.uniform(low=-env.max_step_len,
                #                       high=env.max_step_len,
