...
dyn_rank=2
r_dim=3
model = E2C(obs_dim=obs_dim, z_dim=z_dim, u_dim=u_dim, r_dim=r_dim, env=f'{env_name}', dyn_rank=dyn_rank).to(device)
model.load_state_dict(torch.load('result/pendulum/pendulum_mlp_z5_lam100_rank2_jacloss_kl_r3/model_360', map_location=device))
model.eval()
...
ztarget = state_to_latent(np.array([-0.2, 0.0]))
...
def vis_costs():
    thetas = np.linspace(0, 6.28)
    costs = []
    for t in thetas:
        z = torch.tensor(state_to_latent(np.array([0, 0])), device=device)[None]
        _, _, residuals = model.trans.forward(z, normal.NormalDistribution(z, torch.zeros_like(z)), torch.ones((1, 1)) * t)
        costs.append(residuals.pow(2).sum().detach().cpu().item())
    plt.plot(thetas, costs)
    plt.plot(thetas, thetas ** 2 / 10)
    plt.show()
...
def dynamics(z_np, u_np, t, is_last):
    ...
    Gx = trans.fc_G(h).view(-1, trans.r_dim, trans.z_dim)
    Gu = 0 * trans.fc_H(h).view(-1, trans.r_dim, trans.u_dim)
    g0 = trans.fc_r0(h).view(-1, trans.r_dim)
    residuals = Gx.bmm(z.unsqueeze(-1)).squeeze(-1) + Gu.bmm(u.unsqueeze(-1)).squeeze(-1) + g0
    ...
    residuals = unbatchify(residuals)
...
state = np.array([3.0, 0])

xbar, ubar, K, cost_total = ilqr.do_ilqr(
    dynamics,
    state_to_latent(state),
    z_dim, u_dim, z_dim + u_dim,
    horizon=10, num_iters=25, verbose=False)
...
for i in range(100):
    for j in range(5):
        ...
        xbar, ubar, K, cost_total = ilqr.do_ilqr(
            dynamics,
            z,
            z_dim, u_dim, z_dim + u_dim, horizon=10, num_iters=25, verbose=False,
            ubar=ubar, xbar=xbar, K=K)
    ...
    # plt.imshow(x)
    # plt.show()
ImageSequenceClip(images, fps=20).write_videofile(f'/tmp/ilqr_rank{dyn_rank}_residual{r_dim}_l1.mp4')
